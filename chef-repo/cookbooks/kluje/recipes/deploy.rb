...
# setup directories
['', 'shared', 'shared/tmp', 'shared/pids', 'shared/config', 'shared/log', 'shared/public/assets', 'shared/certs', '.ssh'].each do |dir|
...
deploy "#{node[cookbook_name]['project_dir']}" do
  ...
  before_symlink do
    ...
    rvm_shell 'rake db:migrate' do
      ...
      ruby_string node['rvm']['default_ruby']
      cwd "#{release_path}"
      code "cd #{release_path} && RACK_ENV=#{node.chef_environment} bundle exec rake db:migrate"
    end
  end
  before_restart do
    rvm_shell 'run rake assets:precompile' do
      ...
      ruby_string node['rvm']['default_ruby']
      cwd release_path
      code "RACK_ENV=#{node.chef_environment} bundle exec rake assets:precompile"
    end
  end
  after_restart do
    rvm_shell 'restart resque workers' do
      user 'ubuntu'
      ruby_string node['rvm']['default_ruby']
      cwd release_path
      code "QUEUE='*' RACK_ENV=#{node.chef_environment} bundle exec rake resque:restart_workers >> #{node[cookbook_name]['project_dir']}/shared/log/resque.log 2>&1 &"
    end
  end
end
...
template '/etc/god.conf.d/kluje.god' do
  source 'kluje.god.erb'
  owner 'ubuntu'
  group 'ubuntu'
  mode '0644'
  action :create
